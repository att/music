...
public class DbOperationTriggerHandler implements org.h2.api.Trigger{
	...
	public static void main(String[] args){
		...
		try {
			...
			stmt.execute("CREATE TRIGGER IF NOT EXISTS TRI_INS AFTER INSERT ON TRIGGER_TEST_TABLE FOR EACH ROW CALL \""+DbOperationTriggerHandler.class.getName()+"\"");
			stmt.execute("CREATE TRIGGER IF NOT EXISTS TRI_INS AFTER INSERT ON TRIGGER_TEST_TABLE FOR EACH ROW CALL \""+DbOperationTriggerHandler.class.getName()+"\"");
			stmt.execute("CREATE TRIGGER TRI_UP AFTER UPDATE ON TRIGGER_TEST_TABLE FOR EACH ROW CALL \""+DbOperationTriggerHandler.class.getName()+"\"");
			stmt.execute("CREATE TRIGGER TRI_DEL AFTER DELETE ON TRIGGER_TEST_TABLE FOR EACH ROW CALL \""+DbOperationTriggerHandler.class.getName()+"\"");
